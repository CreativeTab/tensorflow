...
absl::Status DecomposeCollectivePermuteCycle(
  ...
  // auto [back_pairs, fwd_pairs] =
  //     SourceTargetPairsType(cp->source_target_pairs()).SplitEdges(cycle_type);
  ...
  CHECK(recv_data->operand(1) == back_cp || recv_data->operand(2) == back_cp)
      << "Later in the pipeline, CollectivePermuteDecomposer uses post order "
         ...
         "edge is placed before forward edge for more optimal chaining.";
