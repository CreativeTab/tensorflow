...
TransposeSpec GetTransposeSpec(const HloTransposeInstruction* transpose) {
  ...
  if (T1 - T2 == 1) {
    canonical_output_shape.insert(canonical_output_shape.begin() + T1, 1);
    for (auto& p : canonical_permutation) {
      if (p > T2) p++;
    }
    canonical_permutation.insert(canonical_permutation.begin() + T1, T1);
  }
