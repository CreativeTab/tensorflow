...
int64_t GetFlops(const HloDotInstruction& dot) {
  ...
  auto get_non_contracting_dim_sizes =
      [&dot, &dim_size](const proto2::RepeatedField<int64_t> dims,
                        int operand_id) {
        int64_t fmas = 1;
        absl::flat_hash_set<int> contracting_dims = {
            dims.begin(),
            dims.end(),
        };
  ...
  fmas *= get_non_contracting_dim_sizes(
      dot.dot_dimension_numbers().rhs_contracting_dimensions(), 1);
...
absl::StatusOr<DeviceHloInstructionProfiles> MatmulPerfTableGen::Merge(
  ...
  for (const std::string& filename : filenames) {
    // Read file.
    std::string profile_path = absl::StrJoin({filepath, filename}, "/");
